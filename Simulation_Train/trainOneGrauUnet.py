...
# ==========================================
# 3. 改进的网络结构：HDC + Local Refinement
# ==========================================
class ResBlock(nn.Module):
    """标准的残差块 (Dilation=1)，用于局部平滑"""
...
class Actor(nn.Module):
    def __init__(self):
        ...
        # 3. 局部精修 (Local Refinement) - 关键修改！
        # 使用标准的 Dilation=1 卷积。
        # 这一步的作用是：强制让相邻像素“交流”，消除空洞卷积带来的颗粒感。
    ...
    def forward(self, x):
        x = self.entry(x)
        x = self.global_feat(x)  # 此时特征图可能还有“颗粒感”
        x = self.local_refine(x)  # 这一步会把颗粒抹平，变得顺滑
        return self.exit(x)


# ... (Critic & ReplayBuffer 保持不变) ...
...
if __name__ == '__main__':
    ...
    for episode in range(MAX_EPISODES):
        ...
        print(
            f"Ep {episode}: Std={ep_std:.4f} | R_STD={last_reward_info['r_std']:.1f}, R_MSE={last_reward_info['r_mse']:.1f}, R_SSIM={last_reward_info['r_ssim']:.1f}, R_Smooth={last_reward_info['r_smooth']:.1f}")
